...
const word  INA226_CAL_VALUE    = 0x0800;               // INA226 Calibration Register Value (0.1mA/bit)
...
void loop()
{
  ...
  current  = (short)readRegister(INA226_REG_CURRENT);                        // LSB=0.1mA
  power    = (long)readRegister(INA226_REG_POWER) * 25000L;                  // LSB=25mW


  //sprintf(buf, "%.2lf [mV], %.1lf [mA], %.1lf [mW]", (double)voltage / 1000.0, (double)current / 10.0, (double)power);
  sprintf(buf, "%.2lf mV", (double)voltage / 1000.0);
  Serial.println(buf);
